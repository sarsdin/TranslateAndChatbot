...
def main():
    ...
    parser.add_argument("--host", type=str, default="127.0.0.1", help="Server address", required=False)
        ...
        try:
            ...
            if phrase_time and now - phrase_time > timedelta(seconds=phrase_timeout):
                ...
                if args.logdir is not None:
                    ...
                    # pathlib를 사용함으로써 얻는 이점중 args.logdir 같이 파일시스템 경로가 객체처럼 취급가능해진다.
                    # 그래서, /가 pathlib를 이용했을때 경로 구분 문자로써 사용가능하게 되어 직관적이 된다.
                    with (args.logdir / f"{re.sub('[:]','_',now.isoformat())}.wav").open('w+b') as f:
...
def connection_coroutine(host: str, port: int):
    with connect(host, port) as s:
        ...
        while True:
            ...
            # receive
            try:
                length, = struct.unpack("l", recv_exact(4, s))
